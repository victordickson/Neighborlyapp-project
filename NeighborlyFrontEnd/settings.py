#--------- Flask settings
SERVER_HOST = 'https://neighborlyappproj2.azurewebsites.net' # Update this for the appropriate front-end website when up
SERVER_PORT = 5000
FLASK_DEBUG = True # Do not use debug mode in prod

# Flask-Restplus settings
SWAGGER_UI_DOC_EXPANSION = 'list'
RESTPLUS_VALIDATE = True
RESTPLUS_MASK_SWAGGER = False
RESTPLUS_404_HELP = True
API_VERSION = 'v1'

#-------- Azure constants

#API_URL format: "https://proj2funcapp.azurewebsites.net"
<<<<<<< HEAD
API_URL = "https://funcappproj2.azurewebsites.net/api/"
=======
API_URL = "https://myproj2funcapp.azurewebsites.net/"
>>>>>>> 8a4a2484ed39d81712ffdaf44a26877b88d6b19e

# for local host if Azure functions served locally
#API_URL = "http://localhost:7071/api"
